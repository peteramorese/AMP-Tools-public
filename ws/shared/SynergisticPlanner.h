...
public:
    ...
    SynergisticPlanner(const DFA& dfa, const std::unordered_map<uint32_t, abstractionNode>& abstraction, int n, double r, double p, vector<pair<double, double>> controlLimits, DynamicsModel dynamics)
        : ProductAutomaton(dfa, abstraction), KinoRRT(n, r, p, controlLimits, dynamics) {
...
public:
    // Constructor inheriting from ProductAutomaton constructor
    SynergisticPlanner3D(const DFA& dfa, const std::unordered_map<uint32_t, Node3D>& abstraction, int n, double r, double p, vector<pair<double, double>> controlLimits, DynamicsModel dynamics)
        : ProductAutomaton3D(dfa, abstraction), KinoRRT(n, r, p, controlLimits, dynamics) {
        hybridMap = getHybridMap();
        hybridMapBack = getHybridMapBack();
        dynamicWeights = getWeightMap();
        neighborMap = getNeighborMap();
    ...
    amp::Path synergisticPlan(const VectorXd& init_state, MyKinoChecker& collision_checker) {
        ...
        Eigen::Vector3d init;
        init << init_state(0), init_state(2), init_state(4);
        for (const pair<uint32_t, Node3D>& faces : M) {
            std::vector<Eigen::Vector3d> vertices(faces.second.vertices.begin(), faces.second.vertices.end());
        ...
        while (iter < 200)
        {
            ...
            for (auto it = highLevelPlan.begin(); it != last; ++it) {
                ...
                std::cout << "\nGOING TO STATE " << HS << " with region " << di << " and mode " << qi << std::endl;
                std::vector<Eigen::VectorXd> nextRegion(M.at(di).vertices.begin(), M.at(di).vertices.end());
                for (const auto& vec : nextRegion) std::cout << "(" << vec.x() << ", " << vec.y() << ", " << vec.z() << ")" << std::endl;
