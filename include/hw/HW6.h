...
class HW6 {
    public:
        ...
        /// @param verbose Output logs displaying result
        /// @return `true` if path is a valid solution, `false` otherwise
        static bool generateAndCheck(amp::LinkManipulatorMotionPlanner2D& algo, amp::ManipulatorTrajectory2Link& trajectory, amp::Problem2D& prob, bool verbose = true, uint32_t seed = 0u);
