...
public:
    ProductAutomaton(const DFA& dfa, const std::unordered_map<uint32_t, abstractionNode>& abstraction)
        : dfa(dfa), abstraction(abstraction) {
        combineStateSpaces();
...
class ProductAutomaton3D {
public:
    ProductAutomaton3D(const DFA& dfa, const std::unordered_map<uint32_t, Node3D>& abstraction)
        : dfa(dfa), abstraction(abstraction) {
        combineStateSpaces();
        combineTransitionFunctions();
    }
    ...
    const std::unordered_map<uint32_t, Node3D>& getAbstraction() const {
        return abstraction;
    }
...
private:
    const DFA& dfa;
    const std::unordered_map<uint32_t, Node3D>& abstraction;
    std::map<uint32_t, std::pair<uint32_t, uint32_t>> hybridStates;
    std::map<std::pair<uint32_t, uint32_t>, uint32_t> hybridMapBack;
    std::map<uint32_t, std::vector<uint32_t>> neighborMap;
    std::map<uint32_t, std::vector<double>> weightMap;
    ...
    void combineTransitionFunctions() {
        for (const auto state : hybridStates) {
            uint32_t q = state.second.first;
            uint32_t r = state.second.second;
            const Node3D region = abstraction.at(r);
            char o = region.label;
            // std::cout << "Mode State: " << q << std::endl;
            // std::cout << "Observation: " << o << std::endl;
            uint32_t qp = dfa.transition(q, o);
